...
const SingleTooth = ({ id, x, y, data, onInteraction, isSelected }) => {
  ...
  const facePaths = {
    top: `M ${-r1 / Math.sqrt(2)} ${-r1 / Math.sqrt(2)} L ${-r2 / Math.sqrt(2)} ${
      -r2 / Math.sqrt(2)
    } A ${r2} ${r2} 0 0 1 ${r2 / Math.sqrt(2)} ${-r2 / Math.sqrt(2)} L ${
      r1 / Math.sqrt(2)
    } ${-r1 / Math.sqrt(2)} A ${r1} ${r1} 0 0 0 ${-r1 / Math.sqrt(2)} ${
      -r1 / Math.sqrt(2)
    }`,
    bottom: `M ${-r1 / Math.sqrt(2)} ${r1 / Math.sqrt(2)} L ${-r2 / Math.sqrt(2)} ${
      r2 / Math.sqrt(2)
    } A ${r2} ${r2} 0 0 0 ${r2 / Math.sqrt(2)} ${r2 / Math.sqrt(2)} L ${
      r1 / Math.sqrt(2)
    } ${r1 / Math.sqrt(2)} A ${r1} ${r1} 0 0 1 ${-r1 / Math.sqrt(2)} ${
      r1 / Math.sqrt(2)
    }`,
    left: `M ${-r1 / Math.sqrt(2)} ${-r1 / Math.sqrt(2)} L ${-r2 / Math.sqrt(2)} ${
      -r2 / Math.sqrt(2)
    } A ${r2} ${r2} 0 0 0 ${-r2 / Math.sqrt(2)} ${r2 / Math.sqrt(2)} L ${
      -r1 / Math.sqrt(2)
    } ${r1 / Math.sqrt(2)} A ${r1} ${r1} 0 0 1 ${-r1 / Math.sqrt(2)} ${
    ...
    right: `M ${r1 / Math.sqrt(2)} ${-r1 / Math.sqrt(2)} L ${r2 / Math.sqrt(2)} ${
      -r2 / Math.sqrt(2)
    } A ${r2} ${r2} 0 0 1 ${r2 / Math.sqrt(2)} ${r2 / Math.sqrt(2)} L ${
      r1 / Math.sqrt(2)
    } ${r1 / Math.sqrt(2)} A ${r1} ${r1} 0 0 0 ${r1 / Math.sqrt(2)} ${
  ...
  return (
    ...
    >
      ...
      <circle
        ...
        stroke={CONSTANTS.COLORS.BLACK}
        strokeWidth="1"
        fill={faces.center ? getColor(faces.center) : CONSTANTS.COLORS.WHITE}
      />

      <g pointerEvents="none">
        {attributes.crown && (
          <circle r={r2 + 4} fill="none" stroke={getColor(attributes.crown)} strokeWidth="2.5" />
        )}

        {attributes.missing && (
          <g stroke={getColor(attributes.missing)} strokeWidth="3">
            <line x1={-r2} y1={-r2} x2={r2} y2={r2} />
            <line x1={r2} y1={-r2} x2={-r2} y2={r2} />
          </g>
        )}
        ...
        {attributes.implant && (
          <text
            y={r2 + 15}
            textAnchor="middle"
            fill={getColor(attributes.implant)}
            fontWeight="bold"
            fontSize="14"
          >
            I
        ...
        {attributes.endodontics && (
          <text
            y={-r2 - 8}
            textAnchor="middle"
            fill={getColor(attributes.endodontics)}
            fontWeight="bold"
            fontSize="12"
          >
            Tc
...
const Odontograma = ({ initialData, onDataChange }) => {
  ...
  // ✅ NUEVO: toggle herramientas para mobile
  ...
  const getCoords = (id, currentView) => {
    ...
    const offset =
      currentView === 'child'
        ? (ADULTO_UPPER.length - NINO_UPPER.length) * (CONSTANTS.TOOTH_RADIUS + CONSTANTS.GAP / 2)
        : 0;
  ...
  const handleToothInteraction = (e, id) => {
    ...
    if (selectedTool.startsWith('face_') && targetId.startsWith('face-')) {
      ...
      updateData({
        ...
        [view]: {
          ...viewData,
          teethState: { ...viewData.teethState, [id]: { ...tooth, faces: newFaces } },
        },
      });
      return;
    }
    ...
    if (attrMap[toolPrefix]) {
      ...
      updateData({
        ...
        [view]: {
          ...
          teethState: {
            ...viewData.teethState,
            [id]: { ...tooth, attributes: { ...tooth.attributes, [attrKey]: newVal } },
          },
        },
      });
      return;
    ...
    if (selectedTool.startsWith('bridge')) {
      if (!interactionStep) {
        ...
        updateData({
          ...data,
          [view]: { ...viewData, connections: [...viewData.connections, newConn] },
        });
  ...
  return (
    <div className="odontograma-app">
      <div className="view-selector">
        <button
          type="button"
          className={`view-btn ${view === 'adult' ? 'active' : ''}`}
          onClick={() => {
            setView('adult');
            setInteractionStep(null);
          }}
        ...
        <button
          type="button"
          className={`view-btn ${view === 'child' ? 'active' : ''}`}
          onClick={() => {
            setView('child');
            setInteractionStep(null);
          }}
      ...
      {/* ✅ Toggle herramientas (solo se ve en mobile con CSS) */}
      ...
      <div className="odontograma-layout">
        <div className={`controls-panel ${toolsOpen ? 'open' : ''}`}>
          <ToolGroup
            title="Básico"
          ...
          <ToolGroup
            title="Caries/Obt."
            tools={[
              { id: 'face_red', label: '●', className: 'text-red' },
              { id: 'face_blue', label: '●', className: 'text-blue' },
            ]}
            selectedTool={selectedTool}
            onSelect={(id) => {
              setSelectedTool(id);
          ...
          <ToolGroup
            title="Coronas"
            tools={[
              { id: 'crown_red', label: '○', className: 'text-red' },
              { id: 'crown_blue', label: '○', className: 'text-blue' },
            ]}
            selectedTool={selectedTool}
            onSelect={(id) => {
              setSelectedTool(id);
          ...
          <ToolGroup
            title="Pieza Aus."
            tools={[
              { id: 'missing_red', label: 'X', className: 'text-red' },
              { id: 'missing_blue', label: 'X', className: 'text-blue' },
            ]}
            selectedTool={selectedTool}
            onSelect={(id) => {
              setSelectedTool(id);
          ...
          <ToolGroup
            title="TC / Imp"
            tools={[
              { id: 'tc_red', label: 'Tc', className: 'text-red' },
              { id: 'tc_blue', label: 'Tc', className: 'text-blue' },
              { id: 'imp_red', label: 'I', className: 'text-red' },
              { id: 'imp_blue', label: 'I', className: 'text-blue' },
            ]}
            selectedTool={selectedTool}
            onSelect={(id) => {
              setSelectedTool(id);
          ...
          <ToolGroup
            title="Prót. Fija"
            tools={[
              { id: 'bridge_fixed_red', label: '▭', className: 'text-red' },
              { id: 'bridge_fixed_blue', label: '▭', className: 'text-blue' },
            ]}
            selectedTool={selectedTool}
            onSelect={(id) => {
              setSelectedTool(id);
          ...
          <ToolGroup
            title="Prót. Rem."
            tools={[
              { id: 'bridge_removable_red', label: '⟷', className: 'text-red' },
              { id: 'bridge_removable_blue', label: '⟷', className: 'text-blue' },
            ]}
            selectedTool={selectedTool}
            onSelect={(id) => {
              setSelectedTool(id);
          ...
          <svg viewBox="0 0 1000 350" className="odontograma-svg">
            {data[view].connections.map((conn, i) => {
                ...
                return (
                  ...
                  >
                    <rect
                      ...
                      y={yMin}
                      width={xMax - xMin}
                      height={CONSTANTS.TOOTH_RADIUS * 2 + 4}
                      fill="none"
                      stroke={color}
                      strokeWidth="3"
                      rx="4"
                    />
                    <circle cx={(xMin + xMax) / 2} cy={yMin} r="8" fill="#444" />
                    <text
                      x={(xMin + xMax) / 2}
                      y={yMin + 4}
                      textAnchor="middle"
              ...
              return (
                ...
                >
                  <path
                    d={`M ${x1} ${yBase} L ${x1} ${yLine} L ${x2} ${yLine} L ${x2} ${yBase}`}
                    stroke={color}
                    strokeWidth="3"
                    fill="none"
                  />
                  <circle cx={(x1 + x2) / 2} cy={yLine} r="8" fill="#444" />
                  <text
                    x={(x1 + x2) / 2}
                    y={yLine + 4}
                    textAnchor="middle"
            ...
            {currentTeethUpper.map((id) => (
              <SingleTooth
                key={id}
                id={id}
                {...getCoords(id, view)}
                data={data[view].teethState[id] || {}}
                isSelected={interactionStep?.startId === id}
                onInteraction={handleToothInteraction}
              />
            ))}

            <line x1="40" y1="175" x2="960" y2="175" stroke="#eee" strokeWidth="2" />

            {currentTeethLower.map((id) => (
              <SingleTooth
                key={id}
                id={id}
                {...getCoords(id, view)}
